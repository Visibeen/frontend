...
import './SpecialDayEDMs.css';

const templateData = [
  // World Refugee Day
  ['home.jpg', 'hand.jpg', 'car1.jpg'],
  // Happy Father's Day
  ['car2.jpg', 'hand.jpg', 'home copy.jpg'],
  // Happy Mother's Day
  ['home.jpg', 'hand copy.jpg', 'crackers.jpg'],
  // World Wind Day
  ['GreenPeak Marketing.png', 'car4.jpeg', 'Plot  on sale.png'],
];
...
const specialDayTemplateNames = productTemplateNames;
...
const SpecialDayEDMs = () => {
  const location = useLocation();
  const { uploadedLogo, isLogoUploaded, loadAccountInfo, accountInfo, selectedFontStyle } = useAccount();
  const selectedTemplate = location.state?.selectedTemplate || 'Birthday';
  const selectedSpecialDay = location.state?.selectedSpecialDay;
  ...
  // Determine which template names to use and initial index
  let templateNames, initialTemplateIndex;
  if (selectedTemplate) {
    templateNames = productTemplateNames;
    initialTemplateIndex = productTemplateNames.findIndex(name => name.toLowerCase() === selectedTemplate.toLowerCase());
  } else if (selectedSpecialDay) {
    templateNames = specialDayTemplateNames;
    initialTemplateIndex = specialDayTemplateNames.findIndex(name => name.toLowerCase() === selectedSpecialDay.toLowerCase());
  } else {
    templateNames = productTemplateNames;
    initialTemplateIndex = 0;
  }

  const [currentTemplate, setCurrentTemplate] = useState(initialTemplateIndex >= 0 ? initialTemplateIndex : 0);
  ...
  const downloadImage = async () => {
    try {
      ...
      img.onload = () => {
        ...
        const footerElement = previewElement.querySelector('[style*="position: absolute"]');
        if (footerElement && accountInfo) {
          // Calculate footer position and size
          const footerRect = footerElement.getBoundingClientRect();
          const previewRect = previewElement.getBoundingClientRect();
          
          // Calculate relative position
          const footerX = (footerRect.left - previewRect.left) * (canvas.width / previewRect.width);
          const footerY = (footerRect.top - previewRect.top) * (canvas.height / previewRect.height);
          const footerWidth = footerRect.width * (canvas.width / previewRect.width);
          const footerHeight = footerRect.height * (canvas.height / previewRect.height);
  ...
  const leftImg = templateData[currentTemplate][imageOrder[currentTemplate][0]];
  const rightImg = templateData[currentTemplate][imageOrder[currentTemplate][2]];
    ...
    <>
      ...
      <div className="main-content">
        <div className="main-title">
          <div className="products-title">{selectedTemplate ? 'Products EDMs' : selectedSpecialDay ? 'Special Days EDMs' : 'Products EDMs'}</div>
